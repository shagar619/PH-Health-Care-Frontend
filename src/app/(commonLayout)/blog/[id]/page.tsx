...
import { motion } from "framer-motion";
import Link from "next/link";
import { Search, Calendar, ChevronRight, MessageSquare, Eye, ArrowLeft } from "lucide-react";

// --- EXPANDED MOCK DATA ---
// Added 'views', 'comments', and full 'content' paragraphs to make it dynamic.
const allPosts = [
  {
    id: "1",
    title: "10 Tips for Maintaining a Healthy Lifestyle Year-Round",
  ...
  },
  {
    ...
    date: "15 Jun 2025",
    image: "https://images.unsplash.com/photo-1522845015757-50bce044e5da?auto=format&fit=crop&q=80&w=1200",
    views: 2100,
    comments: 89,
  },
  {
  ...
  // Add more IDs matching your listing page here if needed. 
  // The dynamic logic below works for any ID in this array.
];
...
export default function BlogDetailsPage({ params }: { params: { id: string } }) {
  ...
  // DYNAMIC LOOKUP: Find the post based on the URL parameter (params.id)
  const postDetail = allPosts.find((post) => post.id === params.id);

  const handleSearchChange = (query: string) => {
    setSearchQuery(query);
  };

  // If the ID doesn't match any post in our data
  if (!postDetail) {
    return (
      <main className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4">
        <h1 className="text-4xl font-bold text-[#0A3D54] mb-4">Post Not Found</h1>
        <p className="text-slate-500 mb-8">The article you are looking for does not exist or has been removed.</p>
        <Link href="/blog" className="px-6 py-3 bg-[#0A66C2] text-white rounded-lg flex items-center gap-2 hover:bg-[#08529c] transition-colors">
      ...
      <section className="bg-[#0A3D54] py-16 px-4 text-center">
        ...
        >
          ...
          <div className="flex items-center gap-2 text-white/70 text-sm mb-4">
            ...
            <Link href="/blog" className="hover:text-white transition-colors">Blog</Link>
            <ChevronRight className="w-4 h-4" />
            <span className="text-teal-400 line-clamp-1">{postDetail.title}</span>
          </div>
            ...
            <div className="lg:w-2/3">
              ...
              >
                ...
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden p-3 mb-8">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 pb-4 px-2 border-b border-slate-100">
                    <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600">
                      ...
                      <div className="flex items-center gap-1.5 border-l border-slate-200 pl-4">
                        <img src={postDetail.authorAvatar} alt={postDetail.author} className="w-6 h-6 rounded-full object-cover" />
                        <span className="font-medium text-[#0A3D54]">{postDetail.author}</span>
                      </div>
                    </div>
                  ...
                  {/* Body Text (Dynamic based on standard generation) */}
                  <div className="pt-6 px-2 prose prose-slate max-w-none text-slate-600 space-y-6 text-[15px] leading-relaxed pb-4">
                    <p>
                      Welcome to our comprehensive guide on <strong>{postDetail.title.toLowerCase()}</strong>. Maintaining a healthy lifestyle year-round is achievable with consistent habits that support your physical and mental well-being. One of the key pillars is staying informed and making proactive choices. Whether it involves drinking at least eight glasses of water daily, adjusting for factors like weather and physical activity, or adapting to new medical technologies, taking small steps leads to massive long-term results.
                    </p>
                    
                    <p>
                      A balanced diet is equally important, as it fuels your body with the nutrients it needs for energy, growth, and repair. Regular physical activity is another cornerstone. Engaging in at least 150 minutes of moderate exercise per week can boost your cardiovascular health, strengthen muscles, and improve your mood. As {postDetail.author} always emphasizes, prioritizing sleep and rest is just as critical as working out.
                    </p>

                    <blockquote className="border-l-4 border-[#0A66C2] pl-6 py-4 my-8 bg-slate-50 text-slate-700 italic rounded-r-lg font-medium">
                      An extra important note to remember is that consistency is key. Small, sustainable changes in your daily habits will have a more lasting impact than short-term, extreme efforts.
                    </blockquote>
                ...
                <div className="mb-8">
                  ...
                  <div className="bg-white p-6 rounded-xl border border-slate-200 flex flex-col sm:flex-row gap-6 items-start">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-[#0A3D54] mb-2">{postDetail.author}</h4>
                      <p className="text-sm text-slate-600 leading-relaxed">
                        As a certified healthcare professional and wellness coach, {postDetail.author} is passionate about helping others achieve a balanced lifestyle and lasting health. Their journey into health started with their own desire to feel better physically and mentally. They love exploring new ways to stay active, experimenting with treatments, and sharing tips that are practical for busy lives.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <h3 className="text-lg font-bold text-[#0A3D54] mb-4">Article Tags</h3>
                  <div className="flex flex-wrap gap-2">
                    {["Health Tips", postDetail.category, "Wellness", "Care"].map((tag, idx) => (
                      <span key={idx} className="px-4 py-1.5 bg-slate-50 border border-slate-200 text-slate-600 text-xs rounded-lg hover:bg-[#0A66C2] hover:text-white hover:border-[#0A66C2] transition-colors cursor-pointer">
                        {tag}
                      </span>
                    ))}
