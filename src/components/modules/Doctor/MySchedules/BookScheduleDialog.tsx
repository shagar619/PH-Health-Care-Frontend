...
export default function BookScheduleDialog({
     ...
     const [availableSchedules, setAvailableSchedules] = useState<ISchedule[]>(initialAvailableSchedules);
     ...
     toast.success(`Successfully booked ${selectedSchedules.length} schedule${selectedSchedules.length > 1 ? "s" : ""}`);
     if (onSuccess) {
     onSuccess();
     } else {
     router.refresh();
